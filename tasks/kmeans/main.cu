...
__global__ void
determine_nearest_centroid(int batch_size, // How many points to process THIS iteration
...
int main(int argc, const char *argv[]) {
  ...
  #pragma omp parallel num_threads(num_devices)
  {
      ...
      cudaMemcpyAsync(devices[d].points_x, pinned_x + start, devices[d].point_count * sizeof(double), cudaMemcpyHostToDevice, devices[d].stream);
      cudaMemcpyAsync(devices[d].points_y, pinned_y + start, devices[d].point_count * sizeof(double), cudaMemcpyHostToDevice, devices[d].stream);
  ...
  std::vector<double*> h_gather_sum_x(num_devices), h_gather_sum_y(num_devices);
  ...
  cudaMemcpy(d_gather_sum_x, h_gather_sum_x.data(), num_devices * sizeof(double*), cudaMemcpyHostToDevice);
  cudaMemcpy(d_gather_sum_y, h_gather_sum_y.data(), num_devices * sizeof(double*), cudaMemcpyHostToDevice);
  cudaMemcpy(d_gather_count, h_gather_count.data(), num_devices * sizeof(int*), cudaMemcpyHostToDevice);

  const char* mode = (ENABLE_MINI_BATCH && n >= TOTAL_BATCH_SIZE) ? "Mini-Batch" : "Exact";
  printf("Executing k-means à %d iterations (%s mode, %d GPUs)...\n", niters, mode, num_devices);
  ...
  for (int iter = 0; iter < niters; ++iter) {
      ...
      // Pre-compute random offsets BEFORE parallel region (rand() not thread-safe)
      ...
      #pragma omp parallel num_threads(num_devices)
      {
          ...
          determine_nearest_centroid<<<numBlocks, blockSize, 0, devices[d].stream>>>(
      ...
      update_centroid_positions<<<updateNumBlocks, updateBlockSize, 0, devices[0].stream>>>(
